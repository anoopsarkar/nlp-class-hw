...
class AttentionModule(nn.Module):
    ...
    def calcAlpha(self, decoder_hidden, encoder_out):
        ...
        p1 = self.W_enc(encoder_out)
        p2 = torch.cat(p1.shape[0] * [self.W_dec(decoder_hidden)])
        scores = (p1 + p2).tanh().permute(1, 0, 2)
        alpha = torch.nn.functional.softmax(self.V_att(scores), dim=1)
        return alpha
    ...
    def forward(self, decoder_hidden, encoder_out):
        ...
        context = encoder_out.permute(
            1, 2, 0) @ alpha  # (2, 50, 15) @ (2, 15, 1)
        context = context.permute(2, 0, 1)  # (seq, batch, dim)
        alpha = alpha.permute(2, 0, 1)  # (target, batch, source)
