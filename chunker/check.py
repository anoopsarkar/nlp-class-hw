        ...
        for filename in files:
            ...
            if testfile_key in zip_data:
                with open(testfile_path, 'rt') as ref:
                    ref_data = list(filter(lambda k: k, [str(x).strip() for x in ref.read().splitlines()]))
                    output_data = list(filter(lambda k: k, [str(x, 'utf-8').strip() for x in zip_data[testfile_key].splitlines()]))
                    ...
                    if len(ref_data) == len(output_data):
                        ...
                        (prec, recall, tally) = conlleval.evaluate(ref_data, output_data)
