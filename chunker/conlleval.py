...
if __name__ == '__main__':
    ...
    optparser.add_option("-o", "--outputfile", dest="output", default='output.txt', help="output file created by chunker.py [default: output.txt]")
    (opts, _) = optparser.parse_args()

    true_seqs = []
    pred_seqs = []
    with open(opts.ref) as r:
        for sent in read_file(r):
            true_seqs += sent.split()
    with open(opts.output) as o:
        for sent in read_file(o):
            pred_seqs += sent.split()
    print(evaluate(true_seqs, pred_seqs))
